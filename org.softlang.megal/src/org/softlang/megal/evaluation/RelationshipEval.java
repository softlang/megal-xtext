...
public class RelationshipEval extends RelationshipImpl {
	...
	public RelationshipTypeInstance appliedInstance() {
		...
		for (RelationshipType r : allOverloads(megamodel(), getType()))
			for (RelationshipTypeInstance i : r.getInstances()) {
				int dl = i.getLeft().equals(getLeft().getType()) ? 0
						: i.getLeft().latticeBelow(getLeft().getType()) ? 1 : -1;
				int dr = i.getRight().equals(getRight().getType()) ? 0 : i.getRight()
						.latticeBelow(getRight().getType()) ? 1 : -1;
